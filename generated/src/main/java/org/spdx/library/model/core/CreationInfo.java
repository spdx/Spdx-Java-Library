...
public class CreationInfo extends ModelObject  {
	...
	protected List<String> _verify(Set<String> verifiedIds, String specVersion, List<ProfileIdentifierType> profiles) {
		...
		if (createdBys.size() < 1) {
			retval.add("createdBys size " + createdBys.size() + " is less than 1 in CreationInfo");
		}
		if (profiles.size() < 1) {
			retval.add("profiles size " + profiles.size() + " is less than 1 in CreationInfo");
		}
		return retval;
	}
